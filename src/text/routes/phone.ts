...
router.get("/phone/:number", requireAdmin, async (req, res) => {
  const phoneNumber = (req.params.number as string).replace(/[^\d]/g, "");
  const phone = await Phone.findOne({ number: "+1" + phoneNumber });
  if (!phone) {
    throw Error("Phone number does not exist on the list of text subscribers");
  }
  res.send(phone);
});
...
router.post("/phone", requireAdmin, async (req, res) => {
  ...
  if (!["EAST_OAKLAND", "WEST_OAKLAND", "BERKELEY"].includes(region)) {
    res.status(422);
...
router.delete("/phone/:id", requireAdmin, async (req, res) => {
