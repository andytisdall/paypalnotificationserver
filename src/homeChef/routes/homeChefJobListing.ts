...
const getShifts = async (id: string) => {
  const ninetyDaysFromNow = moment().add(90, 'days').format();

  const query = `SELECT Id, GW_Volunteers__Start_Date_Time__c, GW_Volunteers__Number_of_Volunteers_Still_Needed__c from GW_Volunteers__Volunteer_Shift__c WHERE GW_Volunteers__Volunteer_Job__c = '${id}' AND GW_Volunteers__Start_Date_time__c > TODAY AND  GW_Volunteers__Start_Date_time__c <= ${ninetyDaysFromNow}`;
